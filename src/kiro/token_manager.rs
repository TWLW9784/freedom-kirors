...
/// 刷新企业 SSO (external_idp, 如 Azure AD / M365 / Entra ID) Token
///
...
impl DisabledReason {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::AccountThrottled => "AccountThrottled",
    ...
    fn from_str(value: &str) -> Option<Self> {
        match value {
            ...
            "AccountThrottled" => Some(Self::AccountThrottled),
...
impl MultiTokenManager {
    ...
    fn select_next_credential(
        ...
        match mode {
            "balanced" => {
                ...
                // 先按模型发现状态优先（已确认支持目标模型的凭据优先），
                // 再用交叉相乘比较：a/ea < b/eb  ⇔  a*eb < b*ea。
                let (entry, _) = available.iter().min_by(|(a, sa), (b, sb)| {
                    let ra = usize::from(*sa != CachedModelSupport::Confirmed);
                    let rb = usize::from(*sb != CachedModelSupport::Confirmed);
    ...
    fn persist_credentials(&self) -> anyhow::Result<bool> {
        ...
        // persist_lock 已在本函数顶部持有（覆盖快照+序列化+写盘整个临界区，issue #23），
        // 此处不再重复加锁（std Mutex 重入会死锁）。
    ...
    pub async fn force_refresh_token_for(&self, id: u64) -> anyhow::Result<()> {
        {
            ...
            // 同一 refreshToken 拆出的多 profile 副本同步 token rotation。
