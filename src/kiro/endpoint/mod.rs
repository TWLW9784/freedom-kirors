...
pub trait KiroEndpoint: Send + Sync {
    ...
    fn is_account_banned(&self, body: &str) -> bool {
        default_is_account_banned(body)
    }
}
...
pub fn default_is_account_banned(body: &str) -> bool {
    let lower = body.to_ascii_lowercase();
...
mod tests {
    ...
    #[test]
    fn test_default_is_account_banned() {
        ...
        assert!(default_is_account_banned(body));
        ...
        // 仅命中一个短语时不判定为封禁
        assert!(!default_is_account_banned("your account is suspended"));
        assert!(!default_is_account_banned(
