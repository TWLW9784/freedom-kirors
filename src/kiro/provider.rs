        ...
        for attempt in 0..max_retries {
            ...
            if matches!(status.as_u16(), 401 | 403) {
                ...
                if status.as_u16() == 403
                    ...
                    && endpoint.is_account_banned(&body)
                {
                    let has_available = self
                        .token_manager
    ...
    async fn call_api_with_retry(
        ...
        for attempt in 0..max_retries {
                ...
                && endpoint.is_account_throttled(&body)
            {
                ...
                // 本地生产语义：凭据已在 report_account_throttled_for_request 内自动禁用。
                // 同时采用上游 429 传播：账号级风控通常不返回 Retry-After，此时使用
                // 本地实际冷却时间，让下游网关在同一时段内也停止调度该虚拟账号。
